...
ctypedef np.complex_t c_complex

from algorithms.linalg.cblas cimport *
#import scipy.linalg.cython_blas


cdef c_int tensordot_threads = 1
...
IF PARALLEL_TDOT:
	...
	def set_tensordot_threads(n):
		...
		tensordot_threads = n
		if n > 1:
			openmp.omp_set_max_active_levels(2)
...
IF HAS_MKL:
	cdef extern from "mkl.h" nogil:
		ctypedef c_complex MKL_Complex16
ELSE:
	ctypedef c_complex MKL_Complex16
...
cdef inline np.ndarray empty(np.PyArray_Dims dims, c_int type):
	return <np.ndarray>np.PyArray_EMPTY(dims.len, dims.ptr, type, 0 )

cdef inline np.ndarray zeros(np.PyArray_Dims dims, c_int type):
...
def tensordot(a, b, axes = 2):
	...
	cdef list notin_a = [k for k in xrange(nda) if k not in axes_a]
	cdef list newaxes_a = notin_a + axes_a
	cdef list olda = [as_[axis] for axis in notin_a]
	nao = nda - nin
	
	cdef list notin_b = [k for k in xrange(ndb) if k not in axes_b]
	...
	if nao == 0 and nbo == 0:
		return algorithms.linalg.np_conserved.inner(a, b, [ axes_a, axes_b])
...
#	print b.q_dat, "original b.q_dat"
	...
	cdef object c = algorithms.linalg.np_conserved.array(ndc, dtype = None) 	# dtype set later
	...
	if l_dat_a == 1 and l_dat_b == 1:
		...
		for k in xrange(nin): #check if entries match on inner
		...
		if equal: #So contract inner				
			...
			r = 1
			for k in xrange(nin): #inner dimension
				r *= tb.shape[k]  
...
@cython.boundscheck(False)
@cython.nonecheck(False)
cdef  tensordot_guts(object c, c_intp nao, c_intp nbo, c_intp nin, \
		c_int[:, ::1] a_key, \
		c_int[:, ::1] b_key, \
	...
	#For float64 and cfloat64 types, we will call BLAS directly.
	...
	if USE_DRESDEN and (type==np.NPY_DOUBLE or type==np.NPY_CDOUBLE):
		...
		type = -1
	cdef float64 complex cone = 1. + 0j
	cdef float64 complex czero = 0. + 0j
	if TENSORDOT_TIMING:
		t0 = time.time()
...
#	print qob_bnds, "qob"
	...
	while i1 < Nqta and j1 < Nqtb:
		...
		for lex_i in xrange(num_q-1, -1, -1):
	...
	for l in xrange(nao):
		qi = q_ind_a[l]
		i1 = nin+l
		for r in xrange(qoa_shapes.shape[0]):
	...
	for l in xrange(nbo):
		i1 = nin+l
		qi = q_ind_b[i1]
		for r in xrange(qob_shapes.shape[0]):
	...
	cdef int oa_size, ob_size, in_size  #size of combined outer legs / inner leg
	...
	while i1 < Nqta and j1 < Nqtb: #For all charge sectors
		...
		# float64 loop over aouter and bouter, bouter will go through multiple passes, while aouter only goes through one
						...
						if used_aobo: #We've already initialized this sector, so accumulate	
							...
							if type==np.NPY_DOUBLE:
								...
								if oa_size==1 and in_size==1:
									lib_daxpy(ob_size, (<float64*>np.PyArray_DATA(ta))[0], <float64*>np.PyArray_DATA(tb), 1, <float64*>np.PyArray_DATA(tc), 1 )
								elif ob_size==1 and in_size==1:
									lib_daxpy(oa_size, (<float64*>np.PyArray_DATA(tb))[0], <float64*>np.PyArray_DATA(ta), 1, <float64*>np.PyArray_DATA(tc), 1 )
								else:
								"""
								lib_dgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, oa_size, ob_size, in_size, one, <float64*>np.PyArray_DATA(ta), in_size, <float64*>np.PyArray_DATA(tb), ob_size, one, <float64*>np.PyArray_DATA(tc), ob_size )
								...
								"""
								lib_zgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, oa_size, ob_size, in_size, <MKL_Complex16*>&cone, <MKL_Complex16*>np.PyArray_DATA(ta), in_size, <MKL_Complex16*>np.PyArray_DATA(tb), ob_size, <MKL_Complex16*>&cone, <MKL_Complex16*>np.PyArray_DATA(tc), ob_size )
							else:
								tc+=dot(ta, tb)
						else:
							for l in xrange(nao): q_dat_c[dat_len, l] = a_key[i3, nin+l]
							for r in xrange(nbo): q_dat_c[dat_len, nao+r] = b_key[j3,nin+r]
							#GEMM: c -> a A.B + b C , so let a = 1, b = 0

							if type==np.NPY_DOUBLE:
								tc = empty(shpC, type)
								#with nogil:
								lib_dgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, oa_size, ob_size, in_size, one, <float64*>np.PyArray_DATA(ta), in_size, <float64*>np.PyArray_DATA(tb), ob_size, zero, <float64*>np.PyArray_DATA(tc), ob_size )
							elif type==np.NPY_CDOUBLE:
								tc = empty(shpC, type)
								lib_zgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, oa_size, ob_size, in_size, <MKL_Complex16*>&cone, <MKL_Complex16*>np.PyArray_DATA(ta), in_size, <MKL_Complex16*>np.PyArray_DATA(tb), ob_size, <MKL_Complex16*>&czero, <MKL_Complex16*>np.PyArray_DATA(tc), ob_size )
							else:
								tc=dot(ta, tb)
...
cdef void batch_dgemm_accumulate( vector[int] M, vector[int] K, vector[int] N, vector[void*] A, vector[void*] B, vector[void*] C, vector[int] G) nogil:
	...
	while g != G.end():
		lib_dgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, deref(m), deref(n), deref(k), one, <float64*>deref(a), deref(k), <float64*>deref(b), deref(n), zero, <float64*>deref(c), deref(n))
		inc(a)
		inc(b)
		inc(k)

		for i in xrange(deref(g) - 1):
			lib_dgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, deref(m), deref(n), deref(k), one, <float64*>deref(a), deref(k), <float64*>deref(b), deref(n), one, <float64*>deref(c), deref(n))
			inc(a)
			inc(b)
			inc(k)
...
cdef void batch_dgemm_accumulate_parallel( vector[int] M, vector[int] K, vector[int] N, vector[void*] A, vector[void*] B, vector[void*] C, vector[int] G, c_int num_threads) nogil:
	...
	cdef c_int NT = num_threads
	#cdef c_int total_threads = openmp.get_num_threads()
	#omp.mkl_set_num_threads(total_threads/NT)
	for i in prange(size, num_threads = NT, schedule='guided'):
		
		x = starts[i]
		lib_dgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, M[i], N[i], K[x], one, <float64*>(A[x]), K[x], <float64*>(B[x]), N[i], zero, <float64*>(C[i]), N[i])
		x = x + 1

		for j in xrange(G[i] - 1):
			lib_dgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, M[i], N[i], K[x], one, <float64*>(A[x]), K[x], <float64*>(B[x]), N[i], one, <float64*>(C[i]), N[i])
			x = x+1
	#omp.mkl_set_num_threads(total_threads)
...
cdef void batch_zgemm_accumulate( vector[int] M, vector[int] K, vector[int] N, vector[void*] A, vector[void*] B, vector[void*] C, vector[int] G) nogil:
	...
	while g != G.end():
		lib_zgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, deref(m), deref(n), deref(k), <MKL_Complex16*>&cone, <MKL_Complex16*>deref(a), deref(k), <MKL_Complex16*>deref(b), deref(n), <MKL_Complex16*>&czero, <MKL_Complex16*>deref(c), deref(n))
		inc(a)
		inc(b)
		inc(k)

		for i in xrange(deref(g) - 1):
			lib_zgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, deref(m), deref(n), deref(k), <MKL_Complex16*>&cone, <MKL_Complex16*>deref(a), deref(k), <MKL_Complex16*>deref(b), deref(n), <MKL_Complex16*>&cone, <MKL_Complex16*>deref(c), deref(n))
			inc(a)
			inc(b)
			inc(k)
...
cdef void batch_zgemm_accumulate_parallel( vector[int] M, vector[int] K, vector[int] N, vector[void*] A, vector[void*] B, vector[void*] C, vector[int] G, c_int num_threads) nogil:
	...
	for i in prange(size, num_threads = NT, schedule='guided'):
		...
		lib_zgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, M[i], N[i], K[x], <MKL_Complex16*>&cone, <MKL_Complex16*>A[x], K[x], <MKL_Complex16*>B[x], N[i], <MKL_Complex16*>&czero, <MKL_Complex16*>C[i], N[i])
		x = x + 1

		for j in xrange(G[i] - 1):
			lib_zgemm( CblasRowMajor, CblasNoTrans,CblasNoTrans, M[i], N[i], K[x], <MKL_Complex16*>&cone, <MKL_Complex16*>A[x], K[x], <MKL_Complex16*>B[x], N[i], <MKL_Complex16*>&cone, <MKL_Complex16*>C[i], N[i])
			x = x+1
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef combine_legs(self, list axes, pipes_i, qt_conj_i, bint block_single_legs, bint return_pipes = False, bint inplace = False):
	...
	cdef c_int c_ndim = len(axes)
	
	##	set defaults
	if qt_conj_i is None:
		qt_conj_i = np.ones(self.rank, dtype = np.int)
	else:
		qt_conj_i = np.array(qt_conj_i, dtype = np.int)
	...
	if self.labels is not None:
		...
		c_labels = {}
		for i in xrange(c_ndim): #Each new leg
	...
	for i in xrange(c_ndim):	#for each new leg
	...
	if inplace:
		...
		c = algorithms.linalg.np_conserved.array(c_ndim, dtype = self.dtype)
		c.num_q = self.num_q
		c.mod_q = self.mod_q
...
cdef void combine_legs_guts(c, np.ndarray[c_uint, ndim=2, mode="c"] q_dat, list dat, list axes, list pipes, list q_maps, bint inplace):
	...
	for i in xrange(c_ndim): #For each outgoing axis
		qm = q_maps[i]
		...
		for row in xrange(numrow):
			...
			for a in xrange(1, numa): #convert multi-index to flat index
	...
	for i in xrange(c_ndim): #compute the q_dat entry and shape of each out going block
		qt_ind = pipes[i].qt_ind
		
		for iT in xrange(num_dat):
			row = Qt[bounds[iT], i]
			c_q_dat[iT, i] = row
	...
	for iT in xrange(num_dat):
		...
		for row in xrange(bounds[iT], bounds[iT+1]):
			sort_row = sort[row]
...
cpdef transpose_fast(self, np.ndarray[np.npy_intp, ndim=1, mode="c"] axes):
	...
	a = algorithms.linalg.np_conserved.array(self.rank, dtype = self.dtype)
...
@cython.nonecheck(False)
def idaxpy(float64 alpha, np.ndarray x, np.ndarray y):		
	""" y -> y + alpha*x"""
	#cdef float64 alphaD = alpha
	cdef int one = 1
	cdef int size
	if (np.PyArray_ISCONTIGUOUS(x) and np.PyArray_ISCONTIGUOUS(y)) or (np.PyArray_ISFORTRAN(x) and np.PyArray_ISFORTRAN(y)):
		size = np.PyArray_SIZE(x)
		lib_daxpy(size, alpha, <float64*>x.data, one, <float64*>y.data, one )
		return y
	else:
		y+=alpha*x
		return y
...
@cython.nonecheck(False)
def izaxpy(float64 complex alpha, np.ndarray x, np.ndarray y):			
	#cdef float64 complex alphaDC = alpha
	cdef int one = 1
	cdef int size
	if (np.PyArray_ISCONTIGUOUS(x) and np.PyArray_ISCONTIGUOUS(y)) or (np.PyArray_ISFORTRAN(x) and np.PyArray_ISFORTRAN(y)):
		size = np.PyArray_SIZE(x)
		lib_zaxpy(size, <MKL_Complex16*>&alpha, <MKL_Complex16*>x.data, one, <MKL_Complex16*>y.data, one )
		return y
	else:
		y+=alpha*x
		return y
...
cpdef two_norm(a):
	"""Computes 2nrm (Frobenius) of tensor
		Calls BLAS directly for float64s, cfloat64s
	...
	if type==np.NPY_DOUBLE:
		for i in xrange(num):
			t = dat[i]
			size = np.PyArray_SIZE(t)
			c=lib_dnrm2(size, <float64*>t.data, one)
			n+=c*c
	elif type==np.NPY_CDOUBLE:
		for i in xrange(num):
			t = dat[i]
			size = np.PyArray_SIZE(t)
			c=lib_dznrm2(size, <MKL_Complex16*>t.data, one)
			n+=c*c
	else:
		for i in xrange(num):
			t = dat[i]
...
@cython.boundscheck(False)
@cython.nonecheck(False)
def iscal(float64 alpha, a):
	"""Scales a inplace by real float64 'alpha'
		Calls BLAS directly for float64s, cfloat64s
	...
	cdef int size
	if type==np.NPY_DOUBLE:
		for i in xrange(len(a.dat)):
			t = a.dat[i]
			size = np.PyArray_SIZE(t)
			lib_dscal(size, alpha, <float64*>t.data, 1)
	elif type==np.NPY_CDOUBLE:
		for i in xrange(len(a.dat)):
			t = a.dat[i]
			size = np.PyArray_SIZE(t)
			lib_zdscal(size, alpha, <MKL_Complex16*>t.data, 1)
...
def iaxpy(alpha, a, b):
	"""	Optimized inplace axpy operation on npc arrays:
		...
		Calls BLAS directly for a & b float64 or a & b complex float64
	"""
	...
	while i < Na or j < Nb:
		...
		if is_zero: #b is 0
			...
			if is_zero: #a is 0
				...
				size = np.PyArray_SIZE(ta)
				if type==np.NPY_DOUBLE:
					lib_daxpy(size, dalpha, <float64*>(ta.data), one, <float64*>(tb.data), one)
				elif type==np.NPY_CDOUBLE:
					lib_zaxpy(size, <MKL_Complex16*>&zalpha, <MKL_Complex16*>(ta.data), one, <MKL_Complex16*>(tb.data), one)
				else:
					tb = tb+mul(ta, alpha)
				dat.append(tb)
...
def inner(a, b, axes = None, bint do_conj = False):
	...
	cdef list axes_b
	if axes is None:
		axes_a = list(range(a.rank))
		axes_b = list(range(b.rank))
	else:
		axes_a = list(axes[0])
		axes_b = list(axes[1])
	...
	if not equal:
		print "Shapes:", [as_[axes_a[k]] for k in xrange(ndim)], [bs[axes_b[k]] for k in xrange(ndim)]
	...
	cdef float64 complex zc = 0.
	cdef float64 complex zc_temp = 0.
	cdef float64 d_temp
	cdef float64 dc = 0.
	c = 0.
	...
	if BLAS_COMPLEX_INNER and a.dtype.num==np.NPY_CDOUBLE and b.dtype.num==np.NPY_CDOUBLE:
			while i < Na and j < Nb:
				comp = 0
				for lex_i in xrange(ndim-1, -1, -1):
				...
				if comp==1:
					...
					size = np.PyArray_SIZE(ta)
					if do_conj:
						lib_zdotc_sub(size, <void*>(ta.data), one, <void*>(tb.data), one, &zc_temp )
					else:
						lib_zdotu_sub(size, <void*>(ta.data), one, <void*>(tb.data), one, &zc_temp )
					zc+=zc_temp
					i+=1
					j+=1
			c = zc
		...
		while i < Na and j < Nb:
			...
			if comp==1:
				...
				tb = np.PyArray_GETCONTIGUOUS(tb)
				size = np.PyArray_SIZE(ta)
				d_temp = lib_ddot(size, <float64*>(ta.data), one, <float64*>(tb.data), one )
				dc+=d_temp
				i+=1
				j+=1
			c = dc
...
cpdef bint is_blocked_by_charge(np.ndarray[c_int, ndim=2] q_ind):
	...
	for i in xrange(0, length - 1): #For each pair of (sorted) entries
		same = ( q_ind[sort[i], 2] == q_ind[sort[i + 1], 2] ) #I
		for j in xrange(3, num_q + 2):
...
cpdef np.ndarray[c_int, ndim=2, mode='c'] q_sum( \
	...
	for l in xrange(startc + 1, stopc):
		qi = q_ind[l]		# deref leg variables
		qconj = q_conj[l]
		for r in xrange(length):		#i for each entry in q_dat
			for c in xrange(num_q):		#i vectorize across charge
...
@cython.boundscheck(False)
@cython.nonecheck(False)
cdef void copy_strided(char* dest, np.npy_intp* dest_0, np.npy_intp* dest_strides, char* src, np.npy_intp* src_0, np.npy_intp* src_strides, np.npy_intp* len, c_int nd ) nogil:
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef c_int lex_comp_gt(np.ndarray[c_uint, ndim=1] a, np.ndarray[c_uint, ndim=1] b, c_int startc, c_int stopc):
	"""lex - comparison of 1d numpy array

		returns 1 if a > b, 0 if a == b, -1 if a < b
	"""
	cdef c_int i
	for i in xrange(stopc - 1, startc - 1, -1):
		if a[i] > b[i]:
			return 1
		elif a[i] < b[i]:
			return -1
...
@cython.wraparound(False)
@cython.boundscheck(False)
def packVmk(list Vmk_list, np.ndarray[c_float, ndim=6] vmk, np.ndarray[np.uint8_t, ndim=6] mask,  c_int maxM, c_int maxK, spec1, spec2, spec3, spec4, c_float tol):
...
### COMMENTS COMMENTS COMMENTS!!
	...
	cdef c_float v
	cdef c_int n_a = vmk.shape[0]
	cdef c_int n_b = vmk.shape[1]
	cdef c_int n_c = vmk.shape[2]
	cdef c_int n_d = vmk.shape[3]
	for a in xrange(n_a):
		sa = spec1[a]
		for b in xrange(n_b):
			sb = spec2[b]
			for c in xrange(n_c):
				...
				for d in xrange(n_d):
					sd = spec4[d]
					for m in xrange(0, 2*maxM+1):
						m_loc = m-maxM
						for k in xrange(0, 2*maxK+1):		
							v = vmk[a, b, c, d, m, k]
							if abs(v) > tol:
