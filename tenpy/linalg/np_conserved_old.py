...
from tenpy.linalg import svd_dgesvd, svd_zgesvd, _npc_helper
import itertools
import functools
import warnings
import time
...
from tenpy.tools.math import toiterable
from tenpy.tools.math import tonparray
from tenpy.tools.math import anynan
from tenpy.tools.string import joinstr
from tenpy.tools.math import speigs as sp_speigs
...
class array:
	...
	##	rank
		rank = self.rank
		if not (isinstance(rank, int) or isinstance(rank, np.int32) or isinstance(rank, np.int64)): raise RuntimeError("rank is not an int (type=" + str(type(rank)) + ")")
	...
	def save_hdf5(self, hdf5_saver, h5gr, subpath):
		...
		h5gr.attrs["rank"] = self.rank
		h5gr.attrs["shape"] = self.shape
		h5gr.attrs["num_charges"] = self.num_q
		h5gr.attrs["U1_ZN"] = self.mod_q
		h5gr.attrs["total_charge"] = self.charge
		hdf5_saver.save(self.q_ind, subpath + "leg_charges")
		h5gr.attrs["qconj"] = self.q_conj
		hdf5_saver.save(self.dat, subpath + "blocks")
		hdf5_saver.save(self.q_dat, subpath + "block_inds")
		hdf5_saver.save(self.dtype, subpath + "dtype")
		h5gr.attrs["block_inds_sorted"] = self.sorted
		hdf5_saver.save(self.labels, subpath + "labels")
	...
	def from_hdf5(cls, hdf5_loader, h5gr, subpath):
		...
		"""
		rank = hdf5_loader.get_attr(h5gr, "rank")
		obj = cls(rank)
		hdf5_loader.memorize_load(h5gr, obj)
		obj.shape = hdf5_loader.get_attr(h5gr, "shape")
		obj.num_q = hdf5_loader.get_attr(h5gr, "num_charges")
		obj.mod_q = hdf5_loader.get_attr(h5gr, "U1_ZN")
		obj.charge = hdf5_loader.get_attr(h5gr, "total_charge")
		obj.q_ind = hdf5_loader.load(subpath + "leg_charges")
		obj.q_conj = hdf5_loader.get_attr(h5gr, "qconj")
		obj.dat = hdf5_loader.load(subpath + "blocks")
		obj.q_dat = hdf5_loader.load(subpath + "block_inds")
		obj.dtype = hdf5_loader.load(subpath + "dtype")
		obj.sorted = hdf5_loader.get_attr(h5gr, "block_inds_sorted")
		obj.labels = hdf5_loader.load(subpath + "labels")
		obj.check_sanity()
		return obj
	...
	def from_npfunc(cls, func, q_ind, q_conj = None, charge = None, mod_q = None, dtype = None):
		...
		for inds in itertools.product( *[ range(len(q_ind[i]))  for i in reversed(range(d))]) :
			inds = inds[::-1]
	...
	def detect_ndarray_charge(cls, a, q_ind, q_conj = None, mod_q = None):
		...
		# TODO set q_conj
		q_tot = np.empty( num_q, dtype = np.int )
		for qs in itertools.product( *[ range(len(q_ind[i])) for i in reversed(range(d)) ] ):
...
#Bunch / Not bunch
	...
	def to_ndarray(self, perm = None):
		...
		for qs,T in itertools.izip(self.q_dat, self.dat):
			s = tuple( slice( qi[l][ qs[l], 0], qi[l][ qs[l], 1] )  for l in range(d))
			a[s] = T
		return a
...
################################################################################
	...
	def block_shape(self, ind):
		...
		return tuple([ self.q_ind[i][ind[i], 1] - self.q_ind[i][ind[i], 0] for i in range(self.rank) ])
	...
	def block_i(self, qi):
		...
		"""
		## Slow code in python
		#for j in range(len(self.q_dat)): #See if entry is there
	...
	def __getitem__(self, inds):
		"""a[i1, i2, . . . ] returns value of corresponding entry.  The i's are integers (not slices)."""
		if np.isscalar(inds): inds = [inds]
		inds = np.array(inds)
	...
	def __setitem__(self, inds, val):
		"""a[i1, i2, . . . ] = val sets value of corresponding entry.  The i's are integers (not slices)."""
		if np.isscalar(inds): inds = [inds]
		inds = np.array(inds)
		pos = np.array([self.get_q_index(i, inds[i]) for i in range(self.rank)], dtype = np.uint)
...
#		pass
	...
	def take_slice(self, i, axes=0):
		...
		for r in range(len(qind)):
			if i < qind[r, 1]:
				qi = r
				break
		ri = i - qind[qi, 0]		# the relative i within the qi block
		not_axes = [ a for a in range(self.rank) if a != axes ]
		slcharge = mod_onetoinf1D(self.charge - qind[qi, 2:] * self.q_conj[axes], self.mod_q)
		sl = zeros( [ self.q_ind[a] for a in not_axes ], dtype=float64, q_conj=self.q_conj[not_axes], charge=slcharge, mod_q=self.mod_q )
		q_dat = self.q_dat
		sl_q_dat = []
	...
	def take_slice_keeprank(self, inds, axis=0, dependent_axes=[], return_indices=False,verbose=0):
		"""
			given a set of indices on axis "axis", returns a new array with only those indices
			since we are picking out only some indices,  there are many charge sectors that aren't accessed any more
			therefore the other legs of the tensor can be shrunk
			but we don't always want to do this: for example it can be nice not to shrink physical legs so that we can still contract MPOs from the infinite case
			so dependent_axes is a list of axis where we do want to do this shrinking
			it can also be helpful to know the indices on the dependent axes that were kept, so if return_indices is set we return this info
			
		"""
		if not isinstance(axis, int): raise ValueError
		if isinstance(inds, int): inds=[inds]
		if isinstance(dependent_axes, int): dependent_axes=[dependent_axes]
		qind = self.q_ind[axis]

		if verbose>0: print( inds)		
		qi=[]
		ri=[]
		for i in inds:
			for r in range(len(qind)):
				if i < qind[r, 1]:
					if r not in qi:
						qi.append(r)
						ri.append([i - qind[r, 0]])		# the relative i within the qi block
					else:
						ri[qi.index(r)].append(i-qind[r,0])
					break
		...
		sl = zeros( self.q_ind, dtype=self.dtype, q_conj=self.q_conj, charge=self.charge, mod_q=self.mod_q )
		sl.shape[axis]=len(inds)
		sl.sorted=False

		#trim the axis called 'axis'
		q_dat = self.q_dat.copy()
		sl_q_dat = []
		for r in range(len(q_dat)):
			if q_dat[r, axis] in qi:
				i=qi.index(q_dat[r,axis])
				sl_q_dat.append(q_dat[r])
				sl_q_dat[-1][axis]=i
				#sl.dat.append(self.dat[r])
				## TODO, write code to take shit
		sl.q_dat = np.array(sl_q_dat, np.uint)		
		
		sl.q_ind[axis]=np.zeros([len(qi),qind.shape[1]],dtype=int)

		start=0
		for i,q in enumerate(qi):
			sl.q_ind[axis][i,:2]=[start,start+len(ri[i])]
			sl.q_ind[axis][i,2:]=qind[q,2:]
			start+=len(ri[i])
...
#		print sl.q_ind
		...
		for r in range(len(self.q_dat)):
			if self.q_dat[r, axis] in qi:
				i=qi.index(self.q_dat[r, axis])
				tempdat=np.concatenate([np.split(self.dat[r],[rind,rind+1],axis=axis)[1] for rind in ri[i]],axis=axis)
#				print tempdat.shape
#				if len(ri[i])>1: tempdat=tempdat.transpose(range(1,axis+1)+[0]+range(axis+1,self.rank))
#				print tempdat.shape
				dat.append(tempdat)
		sl.dat=dat
		
		out_indices=[]
		#now tries to trim dependent axis
		for dep_axis in dependent_axes:
			if dep_axis==axis: 
				continue
			...
			indmap={}
			temp_indices=[]
			#use q_dat to pick out only the right q_ind sectors
			for q in set(sl.q_dat[:,dep_axis]):
				#make new q_ind row
				new_row=sl.q_ind[dep_axis][q,:]
				diff=new_row[1]-new_row[0]
				temp_indices+=range(new_row[0],new_row[1])
				new_row[0]=start
				new_row[1]=start+diff
				start+=diff
				new_qind.append(new_row)
				
				#get info to update qdat
				indmap[q]=qdat_ind
				qdat_ind+=1
				
			sl.q_ind[dep_axis]=np.array(new_qind)
			sl.shape[dep_axis]=sl.q_ind[dep_axis][-1,1]
			
			#update qdat
			for i in range(sl.q_dat.shape[0]):
				sl.q_dat[i,dep_axis]=indmap[sl.q_dat[i,dep_axis]]
		...
		if return_indices: return sl,out_indices
		else: return sl
	...
	def combine_legs(self, axes, pipes = None, qt_conj = None, block_single_legs = False, timing = False, return_pipes = False):
		...
		return npc_helper.combine_legs(self, axes, pipes, qt_conj, block_single_legs, return_pipes, inplace = False)

	def icombine_legs(self, axes, pipes = None, qt_conj = None, block_single_legs = False, timing = False, return_pipes = False):
		"""	Combine legs together. IN PLACE. See combine_legs.
		"""
		
		return npc_helper.combine_legs(self, axes, pipes, qt_conj, block_single_legs, return_pipes, inplace = True)
	...
	def split_legs(self, legs, leg_pipes, verbose = 0, timing = False):
		...
		for r in range(len(self.q_dat)):		# scan q_dat's rows
			oldrow = self.q_dat[r]
			oldT = self.dat[r]
			...
			entry_range = [ range(s[0], s[1]) for s in qt_map_startstop ]		# list of ranges to iterate over (in qt_map)
			if verbose > 0:
				print ("\told q_dat row:", oldrow, oldT.shape)
				print ("\t", qt_map_startstop, entry_range)
			for qs in itertools.product(*entry_range):		# scan every charge sub sector that is contained in oldrow
				for l in range(nsplit):
					pipe = leg_pipes[l]
				...
				a.dat.append(oldT[tuple(Tslice)].reshape(newshape))
...
class leg_pipe:
	...
	def t_to_mul_ndarray(self, t, qindex=None, a=None):
		...
		if qindex is None:
			...
			for r in qt_map:
				slices = tuple([ slice(q_ind[l][r[2+l],0], q_ind[l][r[2+l],1]) for l in range(self.nlegs) ])
...
def q_flat_from_q_ind(q_inds):
	"""	Take q_inf to q_flat form
		...
		If it is a list, performs operation element-wise
	"""
...
def zeros( q_ind, dtype = float64, q_conj = None, charge = None, mod_q = None ):
	...
	ac.q_ind = [ qi.copy() for qi in q_ind ]
...
def inner(a, b, axes = None, do_conj = False, verbose = 0, timing = False): 
	...
	if axes_a != list(range(nd)):
		a.itranspose(axes_a)
	if axes_b != list(range(nd)):
		b.itranspose(axes_b)
...
def eigh(a, UPLO='L', sort = None, debug = False):
	...
	num_q = a.num_q	
	resw = np.zeros(a.shape[0], dtype = float)
	resv = diag(np.array(1., dtype = np.promote_types(a.dtype, float)), a.q_ind[0], q_conj = a.q_conj[0], mod_q = a.mod_q)
