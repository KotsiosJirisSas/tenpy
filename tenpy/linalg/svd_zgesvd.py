...
import numpy as np
import ctypes
from ctypes import CDLL, POINTER, c_int, byref, c_char, c_double, util
import sys
from numpy.core import array, asarray, zeros, empty, transpose, \
        intc, single, double, csingle, cdouble, inexact, complexfloating, \
        newaxis, ravel, all, inf, dot, add, multiply, identity, sqrt, \
        maximum, flatnonzero, diagonal, arange, transpose, sum, \
        isfinite, size

libs = ["libLAPACK.dylib","libmkl_rt.so","libmkl_intel_lp64.so","liblapack.so", "libopenblas.dll", ctypes.util.find_library('lapack')]
lib = None
for l in libs:
	try:
		lib = CDLL(l)
...
#changed this from fastcopyantranpose since doesnt exist in numpy 3
_fastCT = transpose
