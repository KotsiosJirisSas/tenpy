...
from tenpy.networks.mps import enviro
from tenpy.networks.mps.mps import iMPS 
...
class QH_model(model):
	...
	def measure_density2(self, psi, xs, ys, mu):
		"""	Measures real space "density" psi^D_mu(r) psi_mu(r) at positions [ xs x ys ].
		
			xs and ys are 1D arrays.
			Returns 2D array, shaped (len(xs), len(ys))
			
			
			MZ is in process of doing this correctly for arbitrary bands
		...
		if self.cons_K==1:  #Which m are allowed from conservation?
			ms = [0]
		elif self.cons_K==0:
			ms = np.arange(0, mcut, 1)
		else:
			ms = np.arange(0, mcut, self.cons_K)
		...
		C = np.zeros( (Nflux, nL, len(ms), nL) )   #< c*_{i, a} c_{i+m, b} > = C[i, a, m, b]

		t0 = time.time()
		for a in self.species[mu]:
			for i in np.arange(Nflux):
			
				c = psi.correlation_function(self.AOp, self.aOp, sites2 = (i*N + np.array(ms).reshape((-1, 1))*N + np.array(self.species[mu])).reshape((-1, )), sites1 = [a + i*N], OpStr = OpStr)
			
				C[i, a, :, :] = c.reshape((-1, nL))


		rhoz = np.zeros([len(xs), len(ys)], dtype = complex)

		for i in range(int(ys[0]/self.kappa) - mcut, int(ys[-1]/self.kappa) + mcut):
			for a in range(nL):
				for b in range(nL):
					for j, m in enumerate(ms):
						
							r = np.outer( np.exp(1j*xs*kappa*m), band.phid_phi(a, b, ys - i*kappa, ys - (i+m)*kappa) )*C[i%Nflux, a, j, b]
							if m!=0:
								r*=2
							rhoz += r

		return rhoz.real/self.Lx
	...
	def measure_C(self, psi, z1, z2, Lcut = 3.):
		...
		for n in range(n1L, n1R):
			for m in range(n2L, n2R):
	...
	def pair_correlation(self, psi, max_m = None, xi = 20., verbose=0, remove_disconnected_part = True, exciton_correlator=False):
		"""
			S[mu][nu](qx, qy, dqy) = < :rho_mu(qx, qy) rho_nu(-qx, -qy+dqy): >
			
			max_m is the number of qx modes to compute.
			xi is estimate of correlation length (in the y-dir) in units of ell_B
		...
		D = self.two_body_correlator_pbc(psi, max_m = max_m, max_k = max_k, remove_disconnected_part = remove_disconnected_part, compute_negative_m = True, verbose = verbose)
		k_offset = D['k_offset']
		m_offset = D['m_offset']
		if m_offset!=max_m:
			raise ValueError
		...
		#np.savetxt("temp1",C[0,0,0,0,:,0,:].transpose())
		#np.savetxt("temp2",C[0,0,1,1,:,0,:].transpose())
		...
		if exciton_correlator:
		#return < (c_up^dagger c_dn) (r) (c_dn^dagger c_up) (r') in addition to the default thing
			if len(self.bands)!=2: 
				print( "can only calculate an exciton correlator for a 2-level system!")
				raise ValueError
			names=[]
			for mu,b1 in self.bands.iteritems():
				names.append(b1.__class__.__name__)
			print( names)
			ES={}

			dQy, Qx, Qy1 = np.meshgrid(dqy, qx, qy, indexing='ij')
			Qy2 = Qy1 - dQy

			if names[0]=='LL' and names[1]==names[0]:
				#when normal LLs are used, we just add a contribution every time two layers have the same form factors
				for mu, b1 in self.bands.iteritems():
					for nu, b2 in self.bands.iteritems():
						if nu==mu: continue #if you want the same species you can get it from S
						if b1.anisotropy != b2.anisotropy: 
							print( "this doesn't work for levels with different anisotropies!")
							raise ValueError
						s=0.
						for a,b in itertools.product(range(b1.n), range(b2.n)):
							if a!=b: continue #assuming orthonormality of form factors
							Fab=b1.pre_F(a,b)*b1.F(a,b,Qx,Qy1) #this is where its important that both are of class 'LL'
							Fcd=b1.pre_F(a,b)*b1.F(a,b,-Qx,-Qy2) #this is where its important that both are of class 'LL'
							a=self.species[mu][a]
							b=self.species[nu][b]
							s+=(C[a,b,b,a]*Fab*Fcd).transpose(1,2,0)
						ES[mu]={nu:s}	
			elif names[0]=='general_LL' and names[1]==names[0]:
				#here we can't rely on the individual object's F function
				#we basically have to redo it ourself

				Fab = np.zeros_like( Qx , dtype=np.complex)
				Fcd = np.zeros_like( Qx , dtype=np.complex)
				bands=[]
				for mu,b1 in self.bands.iteritems():
					bands.append(b1)
				print( bands)
				b1=bands[0]
				b2=bands[1]
				if len(b1.u)>1 or len(b2.u)>1: 
					#it is possible for generalLL to have multiple Landau levels, each with a form factor which is a combination of Landau levels
					#this is not currently supported
					raise ValueError

				for a,b in itertools.product(range( len(b1.u[0])), range( len(b2.u[0]) )):
					for k1, v1 in b1.u[0][a].iteritems():
						for k2, v2 in b2.u[0][b].iteritems():
							Fab+=F_NM(k1, k2, Qx, Qy1)*v1*v2
							Fcd+=F_NM(k2, k1, -Qx, -Qy2)*v2*v1
				ES[0]={1:(Fab*Fcd*C[0, 1, 1, 0]).transpose(1,2,0)}
				ES[1]={0:(Fab*Fcd*C[1, 0, 0, 1]).transpose(1,2,0)}
			else:
				print( "code to calcalate exciton correlator given that collection of bands is unavailable")
				raise ValueError


		#this part of the code does the ordinary density-density correlator
		...
		for mu, b1  in self.bands.iteritems():
			...
			for nu, b2 in self.bands.iteritems():
				...
				n_nu = b2.n
				s[nu] = 0.
				for c, d in itertools.product(range(n_nu), range(n_nu) ):
			...
			S[mu] = s
		C = C.transpose([0, 1, 2, 3, 5, 6, 4]) #/why the heck?
        #Order: a b c d qx qy dqy
		outdict={'S':S, 'qx':qx, 'qy':qy, 'dqy':dqy, 'C':C, 'C_abcdxmk':C_abcdxmk, '1body':D['1body']}
		if exciton_correlator: outdict['ES']=ES
		return outdict
	...
	#def plot_g(self, fig, dat, mu, nu, interpolation='bilinear', imshow_kwargs={'cmap':cm.CMRmap}, contour_kwargs={'cmap':cm.gnuplot}, opts={}):
	def plot_g(self, fig, dat, mu, nu, interpolation='bilinear', imshow_kwargs={}, contour_kwargs={}, opts={}):

		""" Plot real-space pair correlation
			dat = {} as returned by convert_S2g   
		"""
		...
		try:
			...
			print( "No matplotlib! Aborting plot_g")
			return None
		
		if fig is None: fig = plt.figure(figsize=(16, 14))
		ax = fig.gca()
		plt_data = np.fft.fftshift(dat['G'][mu][nu].real)
		
		iplt = ax.imshow(plt_data, extent = [dat['y'][0], dat['y'][-1], dat['x'][0], dat['x'][-1]], interpolation=interpolation, **imshow_kwargs)
		cplt = ax.contour(scipy.ndimage.zoom(plt_data, 2), extent = [dat['y'][0], dat['y'][-1], dat['x'][0], dat['x'][-1]], interpolation=interpolation, **contour_kwargs)
		if opts.get('show_colorbar', True):
			if opts.get('subplot_adjust', None) is not None:
				fig.subplots_adjust(right = opts['subplot_adjust'][0])
				cbar_ax = fig.add_axes(opts['subplot_adjust'][1:5])
			else:
				from mpl_toolkits.axes_grid1 import make_axes_locatable
				divider = make_axes_locatable(ax)
				cbar_ax = divider.append_axes("right", size="5%", pad=0.1)        
			cbar = fig.colorbar(iplt, cax=cbar_ax, ticks=np.arange(0,1.5,0.2))
			cbar.solids.set_rasterized(True)
			cbar.solids.set_edgecolor("face")
		if opts.get('show_xlabel', True): ax.set_xlabel('$x$  (along cylinder length)', size=14)
		if opts.get('show_ylabel', True): ax.set_ylabel('$y$  (around cylinder circumference)', size=14)
		ax.set_xlim([dat['x'][0]*1.3, dat['x'][-1]*1.3])
		if opts.get('tight_layout', True): fig.tight_layout(pad=0.1)
		if 'save_to' in opts: fig.savefig(opts['save_to'])
		return fig
	...
	def structure_factor(self, psi, mu, a, xi = 30,nu=None,b=None):
		""" Diagonal structure  of species 'mu' projected into band 'a' with 'nu' projected into band 'b'
			...
			Algorithm:
				...
				S(qy) = <rho_{mu, a}(0, -qy) rho_{nu, b}(0, qy)> / N_e = 
						 F_{mu a; mu a}(0, -qy)  F_{nu b; nu b}(0, qy)  \sum_{x, y} exp(-i kappa qy (y - x) )  < n_{mu, a, x} n_{nu, b, y}> / nu
		...
		if len(G) < Lx*5: #pad with zeros to ensure spacing of dk < 0.1
			G = np.pad(G, (0, int(Lx*5 - len(G)) ), mode = 'constant')

		G = np.hstack([G, G[-1:0:-1]])

		S0 = np.fft.rfft(G) #Returns only positive freq component
		#S = S/np.mean(n1) #structure factor conventionally normalized by Ne, not NPhi
		max = (len(G) + 1)/2
		qy = np.arange(max, dtype = float)/(2*max-1)*Lx
		
		S = S0*self.bands[mu].pre_F(a, a)*self.bands[mu].pre_F(b, b)  #Form factor
...
#						print( val[0],val[1]
	...
	def interpret_two_body_correlator(self, psi, max_m=2, x_limit=10, derivatives=[1], filename='structure', ls=[(0,0)], predict=False, verbose=0, pred_order=10, pred_f=5, pred_skips=10):
		...
		for (l1,l2) in ls:
			corfunc=np.mean(cordict['C'][l1,l1,l2,l2,:,:,:],0)
#			corfunc=corfunc[:,corfunc.shape[1]//2:]
			...
			for m in range(max_m+1):
				outfile=open(filename+str(l1)+str(l2)+'_'+str(m),'w')
				print( outfile,'#0')
	...
	def two_body_correlator_pbc(self, psi, max_m=2, max_k=4, remove_disconnected_part=False, compute_negative_m=False, verbose=0):
	...
	"""
		...
		if self.p_type=='F': sgn = -1		# for fermions
		else: sgn=1
		kOS = max_k		# k_offset
		...
		if remove_disconnected_part:
			for k in range(-max_k, max_k + 1):
				...
				X_ab = np.tensordot(X_ab, np.ones((N, N), dtype=float), axes=0).transpose(0,1,4,5,2,3)
				X_cd = np.tensordot(np.ones((N, N), dtype=float), one_body, axes=0)
	...
	"""
		...
		if m == 0:
			...
			##	Note: We don't have to make all the REnv's, but we do it anyway
			##	- we have the symmetry Cm[:,:,:,:,:,1:] = Cm[:,:,:,:,:,1:].transpose(1,0,3,2,4,5).conj() for m = 0
			##	Case a < b, c < d
			for q in range(1, N):
			...
			if self.p_type=='B' and self.LL_mixing: raise NotImplementedError("Who in their right mind would add LL mixing to a boson problem anyway?  Stay tuned for future updates.")
				...
				for b in range(N):
					for d in range(N):
						if(b!=d or b!=a): corr[a,b,a,d,:,0] = 0 #this operator preserves layer
							...
							if self.p_type=='F':
								corr[a,b,a,b,:,0] = 0		# a = c: the psi^dag's overlap
							else: #SG: boson case
						#TODO, make copies of RE and LE above to avoid recomputing the environments
	...
	"""
		...
		###SG: removed this assertion, added sign at 1516 and computed a=c, b=d terms
				...
				for c in crange:
					for d in drange:
						...
						for i in range(len(Op)):
							...
							OpString[loci] = Op[i]+'Op'			# just for print(ing purposes
							for s in range(prevloci+1, loci):
								OpList[s] = self.StrOp if par == 1 else self.Id
								OpString[s] = 'StrOp' if par == 1 else 'Id'			# just for print(ing purposes
							if Op[i] != 'n': par = 1-par
							prevloci = loci
	...
	def four_body_correlator(self,psi,bR=1,bL=1,cR=0,cL=0,kx=0,muR=0,aR=0,muL=0,aL=0,max_l=10):
		...
		N=self.Nlayer
		cors=np.zeros((len(bL),len(cL),len(bR),len(cR),psi.L/N,max_l),dtype=float)
...
#		spacing=4
#		real_cors=np.zeros((max_l),dtype=float)
...
#					real_cors[y]+=np.exp(-a1**2-(a1+1)**2)*np.exp(-(a2-y-1)**2-(a2-y)**2)
	...
	@classmethod		
	def plot_spectrum(cls, psi, Nsector, bond=-1, catch_error_type=Exception, save_fig=None, subplot=None, limits=(None,None), plot_kwargs={}, plot_pts_filter=None, **kwargs):
	...
	Parameters:
		...
		plot_pts_filter:  A function that processes the data points.  It should take in a (#,2)-array, and return a (#',2)-array.
			...
			- It may be used to rescale the data (e.g. plot_pts_filter = lambda x: 2*x)
	...
	More options:
		print(_spectrum:  Also print( the entanglement spectrum
		print(_spectrum_cutoff:  If print(ing the spectrum, only print( energies < cutoff.
	...
	Returns a dictionary with random tidbits of information."""
		...
		try:
			...
			wh, wv = 1, 0.1
			plt_kwargs = {'markersize':10, 'color':'navy', 'marker':[(-wh, -wv), (wh, -wv), (wh, wv), (-wh, wv)], 'linewidth':0.}
...
##	TODO, make this model root_config
## return this
			pts[:, 0] /= float(psi.L)
			if plot_pts_filter is not None:
				pts = plot_pts_filter(pts)
			if len(pts) == 0: print( "plot_spectrum (N = {}): No points to plot!".format(N))
			info_dict['# pts'] = len(pts)
			if kwargs.get('print_spectrum', False): cls.print_spectrum(psi, bond=bond, charge_filter=tuple(N.tolist()), spec_cutoff=kwargs.get('print_spectrum_cutoff',None))
			...
			if 'title' in kwargs:
				...
				elif isinstance(title, str): plot_title = title.format(N)
	...
	def print_spectrum(cls, psi, bond = -1, charge_filter = None, print_spec = True, print_counting = False, spec_cutoff = None):
		...
		s = ''
		for b,spec in itertools.zip(bond,specs):
	...
	def polarizations_from_RSES(self, psi, truncation_par=None, Nsegments=4, verbose=0, anisotropy=1.):
		""" Computes momentum and charge polarizations from the real space entanglement spectrum
			useful as a check on mompol """
			
		if truncation_par==None: truncation_par={'tol':1e-4, 'chi_max':3600*2} #scott likes these truncation parameters

		psi_temp=psi.copy()
		psi_temp.convert_to_expanded_segment(Nsegments)
		psi2=self.realspace_decomposition(psi_temp,truncation_par=truncation_par,verbose=0,anisotropy=anisotropy)		

		spec=psi2.entanglement_spectrum(psi2.bond_cut)
		out=np.zeros(self.num_q)
		mincharge=None
		for s in spec[0]:
			if mincharge is None or s[0][0]<mincharge:
				mincharge=s[0][0]
			w=np.sum(np.exp(-2*s[1]))
			for c in range(len(s[0])):
				out[c]+=w*s[0][c]
				
		out[0]-=mincharge		
		return np.array(out)/psi.L
	...
	def realspace_decomposition(self, psi, truncation_par=None, start_site=0, end_site=-1, verbose=0,anisotropy=1.):
	...
	##	Split sites
		psi2 = self.split_sites(psi, trunc_par, start_site=start_site, end_site=end_site, verbose=verbose, anisotropy=anisotropy)
	...
	##		sites (a) to (2b-a-1) have been split
		...
		if verbose > 1:
			print( joinstr(["\tphysical charges: "] + [Bs.q_ind[0] for Bs in psi2.B]))
	...
	def split_sites(self, psi, truncation_par = None, start_site=0, end_site=-1, verbose = 0, anisotropy=1.):
		...
		try:
			...
			if self.p_type=='F': origin= int((b+a)/(2.*self.Nlayer))-0.5
			else: origin= int((b+a)/(2.*self.Nlayer))
			print( origin)
	...
	##		erfc(z) = 1 - erf(z).  (limits 2 and 0 respectively)
		for i in range(a, b):
			...
			Bleft, Bright, Y, s_trunc = self.split_site(psi, i, x, trunc_par, charge_dist = charge_dist, verbose = 0, anisotropy=anisotropy)
