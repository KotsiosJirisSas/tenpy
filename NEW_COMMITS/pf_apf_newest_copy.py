...
def patch_WF_together(psi1,psi2,sites,pstate):
    ...
    B_new,shiftara=set_MPS_boundary(legL,leg_physical,legR,max_right,pstate)
    print('REAL SHIFTARA',shiftara)
...
def set_MPS_boundary(legL,leg_physical, legR,max_right,pstate=[]):
    ...
    shiftara=L[np.random.randint(len(L))]-max_right
    print(np.random.randint(len(L)))
    for i in range(2+len(pstate)):
        leg_physical_site=leg_physical[i]
        ch_physical=leg_physical_site.to_qflat()[0]
        shiftara+=ch_physical
...
def bulk_bulk_boundary(name_load,name_save,name_graph,pstate=[]):
    ...
    #Load left hand side
    psi_halfinf_right=load_data(loaded_xxxx2,sites[len(pstate)+half:],shift=len(pstate)+half,side='right',charge_shift=[0,0])
    ...
    distance=np.arange(len(filling))
    K_sector=np.sum((filling-1/2)*distance)

    print('K sector')
    print(K_sector)
    print('electron number')
    print(np.sum(filling-1/2))
    if np.abs(np.sum(filling-1/2))>0.05:
        quit()
