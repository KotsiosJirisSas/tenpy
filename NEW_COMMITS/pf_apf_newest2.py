...
def set_MPS_boundary(legL,leg_physical, legR,pstate=[]):
    ...
    if len(pstate)==0:
        ...
        for i in range(len(pstate)):
            ...
            qflat=[]
            if pstate[i]=='empty':# or pstate[i]=='full':
                #add 0 on our site
                ...
                #sort qflat2
                qflat=np.array(qflat)
                sorted_charges = np.lexsort(np.array(qflat).T[::-1]) 
                ...
                #print(np.random.random(len(ind)))
            elif pstate[i]=='full':#or pstate[i]=='empty':
                #add one on our site
...
def bulk_bulk_boundary(name_load,name_save,name_graph,pstate=[]):
    ...
    #Load left hand side
    psi_halfinf_right=load_data(loaded_xxxx2,sites[len(pstate)+1+half:],shift=len(pstate)+half+1,side='right',charge_shift=[0,0])
