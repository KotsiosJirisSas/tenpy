...
def bulk_bulk_boundary(name_load,name_save,name_graph,pstate=[]):
    ...
    filling=psi_halfinf_right.expectation_value("nOp")
    print('filling after the projection')
    print(filling)
    N_projected=np.sum(filling)
    ...
    psi_halfinf_left=load_data(loaded_xxxx,sites[:half],shift=-half,side='left',charge_shift=charge_shift)
    filling=psi_halfinf_left.expectation_value("nOp")
    ...
    psi_total,charge=project_side_of_mps(psi_total,side='left')
    psi_total,charge=project_RHS_of_mps_vac( psi_total)
    ...
    #leg_MPS
    leg_MPS=psi_total._B[-1].get_leg('vR')
    #leg_MPO
    leg_MPO=M.H_MPO._W[-1].get_leg('wR')
    ...
    name_env_q='Envs_qs'
    data=load_environments_from_file(name,name_env_q,side='left')
    left_environment=load_environment(data,-half-1,root_config_,conserve, perm[0],side='left',old=True,charge_shift=charge_shift)
    #print(M.H_MPO._W[0])
    #print('psi'*100)
...
name_save='fourth_eigen_Ly=18_L=402_FINITE_pf_apf_'+str(pstate[:4])+'_num='+str(len(pstate))
print("#"*100)
